import numpy as np
from DIP.others.constants import MAX_PIXEL_VALUE, MIN_MAX_PIXEL_VALUE
...
def addSaltAndPepperNoise(img, saltProb=0.01, pepperProb=0.01):
    ...
    if img.ndim == 3:
        res[saltMask] = (MAX_PIXEL_VALUE, MAX_PIXEL_VALUE, MAX_PIXEL_VALUE)
        res[pepperMask] = (MIN_MAX_PIXEL_VALUE, MIN_MAX_PIXEL_VALUE, MIN_MAX_PIXEL_VALUE)
    elif img.ndim == 2:
        res[saltMask] = MAX_PIXEL_VALUE
        res[pepperMask] = MIN_MAX_PIXEL_VALUE
...
def addGaussianNoise(img, a=0, sigma=10):
    ...
    return np.clip(tmp, 0, MAX_PIXEL_VALUE).astype("uint8")
...
def addPoissonNoise(img, scale=1.0):
    img = img.astype('float64') / MAX_PIXEL_VALUE
    img = img * scale
    noisy = np.random.poisson(img)
    noisy = (noisy / scale) * MAX_PIXEL_VALUE
    return np.clip(noisy, 0, MAX_PIXEL_VALUE).astype("uint8")